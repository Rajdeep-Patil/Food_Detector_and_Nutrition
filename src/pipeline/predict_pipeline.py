...
_transform = transforms.Compose([transforms.Resize(256),transforms.CenterCrop(224),transforms.Normalize(mean=IMAGENET_MEAN,std=IMAGENET_STD),])
    ...
    def predict(self, image_path: str, threshold: float = 0.5):
        ...
        try:
            ...
            with torch.no_grad():
                ...
                predicted = (outputs > threshold).float().squeeze()

            predicted_labels = [

                self.class_names[i]

                for i, v in enumerate(predicted.cpu().numpy())if v == 1.0]

            scores = outputs.squeeze().cpu().numpy()

            top5_idx = scores.argsort()[::-1][:5]

            top5 = [(self.class_names[i],float(scores[i]))for i in top5_idx]
